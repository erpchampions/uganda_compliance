...
from uganda_compliance.efris.api_classes.e_invoice import (
    EInvoiceAPI,
    decode_e_tax_rate,
)
...
class EInvoice(Document):
    ...
    def before_submit(self):
        ...
        if not self.irn:
            ...
            msg += _(
                "You must generate EFRIS for the sales invoice to submit this e-invoice."
            )
    ...
    def validate_uom(self):
        for item in self.items:
            ...
            if not efris_uom_code:
                msg = (
                    _("Row #{}: {} has invalid UOM set.").format(
    ...
    def set_tax_details(self):
        ...
        for tax_item in self.sales_invoice.taxes:
            ...
            if tax_item.charge_type == "On Net Total" and (
                accnt_id == output_vat_account or accnt.account_type == "Tax"
            ):
                e_taxes_table = {}
                for e_invoice_item in self.items:
                    ...
                    if self.sales_invoice.additional_discount_percentage:
                        discount_amount = e_invoice_item.amount * (
                            self.sales_invoice.additional_discount_percentage / 100
                        )
                        # Adjust gross amount by subtracting the discount amount
                        gross_amount = round(
                            (e_invoice_item.amount - discount_amount), 2
                        )
                    else:
                    ...
                    tax_rate = decode_e_tax_rate(
                        e_invoice_item.gst_rate, e_tax_category
                    )
    ...
    def set_payment_details(self):
        ...
        if self.sales_invoice.is_return:
            orignal_einvoice = EInvoiceAPI.get_einvoice(
                self.sales_invoice.return_against
            )
        ...
        for pay_amount in self.sales_invoice.payments:
            ...
            e_payments = frappe._dict(
                {"amount": paid_amount, "mode_of_payment": payment_mode}
            )
    ...
    def set_seller_details(self):
        ...
        if company_address:
            seller_address = frappe.get_all(
                "Address", {"name": company_address}, ["*"]
            )[0]
            self.seller_phone = seller_address.phone
            self.seller_address = " ".join(
                filter(
                    None,
                    [
            ...
            if not self.seller_address:
                frappe.throw("Seller Address is not set")
    ...
    def set_buyer_details(self):
        ...
        if not efris_customer_type:
            frappe.throw(
                _("EFRIS Customer Type must be set on on the Customer Details page.")
            )

        if efris_customer_type == "B2B" and not self.buyer_gstin:
            frappe.log_error(
                _(
                    "TaxID/TIN must be set for B2B Customer (GST Category). See Tax tab on Customer profile."
                )
            )
            frappe.throw(
                _(
    ...
    def set_additional_discounts(self):
        ...
        self.additional_discount_percentage = (
            self.sales_invoice.additional_discount_percentage
        )
    ...
    def fetch_items_from_invoice(self):
        ...
        for i, item in enumerate(self.sales_invoice.items):
            if not item.efris_commodity_code:
                frappe.throw(
                    _(
                        "Row #{}: Item {} must have EFRIS Commodity code set to be able to generate e-invoice."
            ...
            if item_doc.taxes:
                ...
                frappe.throw(
                    _(
                        "Row #{}: Item {} must have Tax Template set under Tax tab"
                    ).format(item.idx, item.item_code)
                )
            ...
            einvoice_item = frappe._dict(
                {
                    ...
                    "gst_rate": decode_e_tax_rate(
                        round(item_taxes[i].rate / 100, 2), efris_tax_category
                    ),
                    ...
                    "commodity_code_description": frappe.get_doc(
                        "EFRIS Commodity Code", item.efris_commodity_code
                    ).commodity_name,
                }
            )
        ...
        for row in self.items:
            ...
            if discount_percentage > 0:
                (
                    ...
                    discounted_item,
                    discountTaxRate,
                ) = self.calculate_discounts(row, discount_percentage, taxRate)
    ...
    def get_payment_details(self):
        ...
        for row in self.e_payments:
            ...
            if mode_of_payment and mode_of_payment != CONST_EFRIS_PAYMENT_MODE_CREDIT:
                efris_payment_mode = frappe.db.get_value(
                    "Mode of Payment", {"name": mode_of_payment}, "efris_payment_mode"
                )
                if not efris_payment_mode:
                    frappe.throw(
                        f"EFRIS Mode of Payment must be configured on Payment Mode: {mode_of_payment}"
                    )
...
def calculate_additional_discounts(invoice):
    ...
    item_taxes = doc.item_wise_tax_details
    total_item_tax, total_discount_tax = _calculate_taxes_and_discounts(
        doc, item_taxes, discount_percentage
    )
...
def _get_valid_document(invoice):
    ...
    try:
        ...
        frappe.log_error(
            f"Failed to process document: {e}", "calculate_additional_discounts Error"
        )
        return None
...
def _calculate_taxes_and_discounts(doc, item_taxes, discount_percentage):
    ...
    for idx, row in enumerate(doc.get("items", [])):
        ...
        tax_rate = float(item_taxes[idx].rate) or 0.0

        discount_tax, item_tax = _calculate_tax_adjustments(
            discount_amount, tax_rate, row.amount
        )
        total_item_tax += item_tax
        total_discount_tax += discount_tax

        _update_row_with_discount_details(
            row, discount_amount, discount_tax, tax_rate, item_tax
        )
...
def _update_row_with_discount_details(
    row, discount_amount, discount_tax, tax_rate, item_tax
):
    """
