...
class TestPOSEInvoice(unittest.TestCase):
	def setUp(self):
	...
	def test_item_validations(self):
		...
		self.assertEqual(item.efris_commodity_code, '990002')
		# if commodity_code starts with 99 then its a service item
		self.assertTrue(item.is_service_item)
		self.assertEqual(item.quantity, 2000)

		item = self.e_invoice.items[1]
		self.assertEqual(item.efris_commodity_code, '890002')
		# if commodity_code doesn't starts with 99 then not a service item
...
def make_e_invoice():
	...
	sales_invoice.items[0].efris_commodity_code = '990002'
	sales_invoice.items[1].efris_commodity_code = '890002'
	sales_invoice.save()
