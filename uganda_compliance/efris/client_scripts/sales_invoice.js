...
frappe.ui.form.on('Sales Invoice', {
                    ...
                    add_einvoice_button(__('Check EFRIS Approval Status'), async () => {
                        ...
                        try {
                            await frappe.call({
                                method: 'uganda_compliance.efris.api_classes.e_invoice.confirm_irn_cancellation',
                                args: { sales_invoice: frm.doc },
                                freeze: false, // Avoid freezing the UI
                                callback: function(r) {
                                    if (!r.exc) {
                                        frm.reload_doc(); // Reload the form here
                                    }
    ...
    on_submit: function(frm) {
        // Reload the form only after the submit operation is complete
        setTimeout(() => {
            frm.reload_doc();
        }, 1000); // Delay the reload to ensure the submit operation completes
    }
});
