        ...
        if (einvoice_status === "EFRIS Credit Note Pending") {
          frm.add_custom_button(__("Check EFRIS Approval Status"), async () => {
            ...
            try {
              await frappe.call({
                method:
                  "uganda_compliance.efris.api_classes.e_invoice.confirm_irn_cancellation",
    ...
    frappe.call({
      ...
      callback: function (response) {
        if (response.message && response.message.length > 0) {
          const efris_payment_mode_name =
            response.message[0].efris_payment_mode;
          ...
          frappe.call({
            ...
            callback: function (response) {
              const include_payment_flag =
                response.message?.[0]?.include_payment || 0;
  ...
  if (frm.doc.efris_invoice && !frm.doc.is_return) {
    try {
      const response = frappe.call({
        method:
          "uganda_compliance.efris.doctype.e_invoicing_settings.e_invoicing_settings.get_e_tax_template",
...
function handle_update_stock_setting(frm) {
  ...
  if (is_efris_invoice) {
    frappe.call({
      method:
        "uganda_compliance.efris.doctype.e_invoicing_settings.e_invoicing_settings.get_e_company_settings",
...
async function add_custom_buttons(frm) {
  ...
  const auto_send_submitted_invoice =
