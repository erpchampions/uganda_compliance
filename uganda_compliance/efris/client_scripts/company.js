frappe.ui.form.on('Company', {
            ...
            frappe.call({
                ...
                callback: function(r) {
                    if (r.message && r.message.length > 0) {
                        ...
                        // Show the field if it's not a new document and tax_id or nin_or_brn is set
                        if (frm.doc.tax_id || frm.doc.nin_or_brn) {
                            frm.set_df_property('company_sync_efris', 'hidden', 0);  // Show field
                        } else {
                            frm.set_df_property('company_sync_efris', 'hidden', 1);  // Hide field if both are empty
                        }
                    } else {
                        ...
                        frm.set_df_property('company_sync_efris', 'hidden', 1);  // Hide field
                    }
                }
            });
        } else {
            // Hide the field if it's a new document
            frm.set_df_property('company_sync_efris', 'hidden', 1);
        }
    },
    tax_id: function(frm) {
    ...
    nin_or_brn: function(frm) {
        // Recheck when nin_or_brn is modified
        frm.trigger('refresh');
    }
});
